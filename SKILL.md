---
name: mmkc-tiktok-search-skill
description: >
  使用本地真实 Chrome 登录态对 TikTok 可见页面进行只读调研，整理链接、作者、指标、截图和 JSON/CSV/Markdown 结果。
  English: Run read-only TikTok research through a local logged-in Chrome session, collecting links, authors, metrics, screenshots, and JSON/CSV/Markdown outputs.
---
...
4. **后续衔接**：
   - 要下载视频 → 交给 `mmkc-tiktok-downloader-skill`
   - 要分析视频 → 交给 `mmkc-tiktok-readvideo-skill`
...
```text
mmkc-tiktok-search-skill/projects/tiktok-research/YYYYMMDD_HHMMSS_<slug>/
